...
productRouter.get("/product/:id", async (req: Request, res: Response) => {
    try {
        ...
        if (!product) {
            return res.status(StatusCodes.NOT_FOUND).json({ error: `Product with ID ${productId} not found.` }); // Use backticks for template literals
        }
...
productRouter.delete("/product/:id", async (req: Request, res: Response) => {
    try {
        ...
        return res.status(StatusCodes.OK).json({ msg: `Product with ID ${productId} deleted.` }); // Use backticks for template literals
    } catch (error) {
        return res.status(StatusCodes.INTERNAL_SERVER_ERROR).json({ error: "Internal Server Error" });
    }
});
